...
import { Color, Space } from '../source/color';
import { ArrayType, ColorScale, InterpolationHint } from '../source/colorscale';
...
describe('ColorScale', () => {
    ...
    before(() => {
        ...
        colors = [255, 0, 255];
        let stepCount = 7;
        oneColorScale = ColorScale.fromArray(colors, ArrayType.RGB, stepCount);
        colors = [255, 0, 128, 121, 12, 42, 21, 0, 90];
        stepCount = 7;
        positions = [2, 0, 1];
        defaultColorScale = ColorScale.fromArray(colors, ArrayType.RGB, stepCount, positions);
        emptyColorScale = ColorScale.fromArray([], ArrayType.RGB, 0);
    ...
    it('should be creatable from an array with positions', () => {
        const colorScale = ColorScale.fromArray(colors, ArrayType.RGB, 2, positions);
        /* eslint-disable-next-line no-unused-expressions */
        expect(colorScale).to.not.be.undefined;
    });

    it('should be creatable from an array with positions', () => {
        const colorScale = ColorScale.fromArray(colors, ArrayType.RGB, 1, positions);
        /* eslint-disable-next-line no-unused-expressions */
        expect(colorScale).to.not.be.undefined;
    });

    it('should be creatable from an array with alpha values', () => {
        const colorScale = ColorScale.fromArray([0.5, 0.5, 0.5, 0.2], ArrayType.RGBA, 1);
        /* eslint-disable-next-line no-unused-expressions */
        expect(colorScale).to.not.be.undefined;
    });

    it('should be creatable from float an array', () => {
        const colorScale = ColorScale.fromArray([0.5, 0.5, 0.5], ArrayType.RGBf, 1);
        /* eslint-disable-next-line no-unused-expressions */
        expect(colorScale).to.not.be.undefined;
    });

    it('should be creatable from float an array', () => {
        const colorScale = ColorScale.fromArray([0.5, 0.5, 0.5], ArrayType.RGBf, 3);
        /* eslint-disable-next-line no-unused-expressions */
        expect(colorScale).to.not.be.undefined;
    });
    ...
    it('should be creatable from an array with alpha values', () => {
        const colorScale = ColorScale.fromArray([0.5, 0.5, 0.5, 0.2], ArrayType.RGBAf, 1);
        /* eslint-disable-next-line no-unused-expressions */
        expect(colorScale).to.not.be.undefined;
    });

    it('should be creatable from an array with empty arrays', () => {
        const colorScale = ColorScale.fromArray([], ArrayType.RGB, 0);
        /* eslint-disable-next-line no-unused-expressions */
        expect(colorScale).to.not.be.undefined;
    });

    it('should be linear interpolate-able with only one color', () => {
        const interpolatedColor = oneColorScale.lerp(1, Space.RGB);
        /* eslint-disable-next-line no-unused-expressions */
        expect(interpolatedColor).to.not.be.undefined;
        if (interpolatedColor) {
            expect(interpolatedColor.tuple(Space.RGB, false))
                .to.eql(
                    color.tuple(Space.RGB, false));
        }
    });

    it('should be linear interpolate-able', () => {
        const interpolatedColor = defaultColorScale.lerp(-0.1, Space.RGB);
        /* eslint-disable-next-line no-unused-expressions */
        expect(interpolatedColor).to.not.be.undefined;
        if (interpolatedColor) {
            expect(interpolatedColor.tuple(Space.RGB, false))
    ...
    it('should be undefined when calling lerp on ColorScale without colors', () => {
        const interpolatedColor = emptyColorScale.lerp(1, Space.RGB);
        /* eslint-disable-next-line no-unused-expressions */
    ...
    it('should be able to turn to UInt8 bits', () => {
        let uint8Array = defaultColorScale.bitsUI8(Space.RGB, false);
        expect(uint8Array.length).to.eq(defaultColorScale.length * 3);

        uint8Array = defaultColorScale.bitsUI8(Space.RGB, true);
        expect(uint8Array.length).to.eq(defaultColorScale.length * 4);

        uint8Array = emptyColorScale.bitsUI8(Space.RGB, false);
        expect(uint8Array.length).to.eq(0);
    });

    it('should be able to turn to float32 bits', () => {
        let uint8Array = defaultColorScale.bitsF32(Space.RGB, false);
        expect(uint8Array.length).to.eq(defaultColorScale.length * 3);

        uint8Array = defaultColorScale.bitsF32(Space.RGB, true);
        expect(uint8Array.length).to.eq(defaultColorScale.length * 4);

        uint8Array = emptyColorScale.bitsF32(Space.RGB, false);
        expect(uint8Array.length).to.eq(0);
    });
